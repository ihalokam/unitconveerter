...
export default function HeroCalc() {
    ...
    // imperial inputs (feet/inches), metric inputs (meters/cm)
    const [length, setLength] = useState("10");
    const [width, setWidth] = useState("10");
    const [thicknessIn, setThicknessIn] = useState("4"); // inches or cm
    const [diameterIn, setDiameterIn] = useState("12"); // inches, for round column
    const [heightFt, setHeightFt] = useState("6"); // feet or meters, round column height
    ...
    const result = useMemo(() => {
        ...
        const T = parseFloat(thicknessIn) || 0; // inches (imperial) or cm (metric)
        const D = parseFloat(diameterIn) || 0; // inches (imperial) or cm (metric)
        const H = parseFloat(heightFt) || 0; // feet (imperial) or meters (metric)
        ...
        if (!isMetric) {
            switch (shape) {
                ...
                case "circular": {
                    const rFt = D / 2 / 12; // diameter field doubles as slab diameter (in)
                    cuFt = Math.PI * rFt * rFt * (T / 12);
                    break;
                }
            }
        } else {
            // metric: compute cubic meters directly, then convert to cu ft for shared math
            let cuM = 0;
            switch (shape) {
                case "slab":
                case "footing":
        ...
        const weightLbs = cuFtWithWaste * 150; // ~150 lb/ft3 wet concrete

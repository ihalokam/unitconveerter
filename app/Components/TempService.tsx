...
/**
 * TemperatureConverter.tsx — Self-contained, single-file temperature unit converter
...
type TempUnit = "c" | "f" | "k" | "r" | "re";

type RoundMode = "exact" | "integer" | "decimal";
type Step = "upload" | "header" | "columns" | "output" | "done";

const TEMP_UNITS: { value: TempUnit; label: string; symbol: string; category: string }[] = [
    { value: "c", label: "Celsius", symbol: "°C", category: "Common" },
    { value: "f", label: "Fahrenheit", symbol: "°F", category: "Common" },
    { value: "k", label: "Kelvin", symbol: "K", category: "Scientific" },
    { value: "r", label: "Rankine", symbol: "°R", category: "Scientific" },
    { value: "re", label: "Réaumur", symbol: "°Ré", category: "Historical" },
];

/** Temperature uses formulas, not linear factors. Convert via Celsius as intermediate. */
...
function safeConvert(
    ...
    return applyRounding(convertTemp(val, from, to), mode, decimals);
}
...
const symOf = (v: string) => TEMP_UNITS.find((u) => u.value === v)?.symbol ?? v;
          ...
          ${loading ? "pointer-events-none opacity-60" : ""}`}
            ...
            <div className="w-full max-w-lg rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
                ...
                <ol className="flex flex-col gap-2">
                    {[
                        "Upload a CSV or Excel file with temperature data",
...
function UnitSelect({ value, onChange, label }: { value: TempUnit; onChange: (v: TempUnit) => void; label: string }) {
    const categories = [...new Set(TEMP_UNITS.map((u) => u.category))];
    return (
        <div className="flex flex-col gap-1">
            <label className="text-xs text-zinc-500 font-medium">{label}</label>
            <select value={value} onChange={(e) => onChange(e.target.value as TempUnit)}
          ...
          focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-colors">
                {categories.map((cat) => (
                    <optgroup key={cat} label={cat}>
                        {TEMP_UNITS.filter((u) => u.category === cat).map((u) => (
...
function ColumnsStep({ columns, sampleRows, initialRules, onConfirm, onBack }: {
    ...
    const addRule = () => {
        ...
        setRules((p) => [...p, { id: uid(), column: unused ?? columns[0] ?? "", from: "c", to: "f", newColumnSuffix: false }]);
        setError(null);
    };
    ...
    return (
        <section className="flex flex-col gap-6">
            <div>
                ...
                <p className="text-sm text-zinc-500">Select which columns hold temperature values and specify their current and target units.</p>
            </div>
            ...
            {suggestions.length > 0 && (
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 px-4 py-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {suggestions.map((col) => (
                            <button key={col}
                                onClick={() => setRules((p) => [...p, { id: uid(), column: col, from: "c", to: "f", newColumnSuffix: false }])}
                    ...
                    focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-colors">
                        ...
                        {rule.column && sampleRows[0] && sampleRows[0][rule.column] != null && (
                            <div className="mt-3 pt-3 border-t border-zinc-800 flex items-center gap-2 text-xs text-zinc-500 flex-wrap">
                                ...
                                {sampleRows.slice(0, 3).map((r, i) => {
                                    ...
                                    const result = safeConvert(raw, rule.from, rule.to, "decimal", 2);
                                    return (
                        ...
                        {rule.newColumnSuffix && (
                            <p className="mt-2 text-xs text-zinc-500">
                                A new column <span className="text-zinc-300 font-mono">{rule.column}_{symOf(rule.to).replace("°", "")}</span> will be appended.
                            </p>
                        )}
                    </div>
                ))}
        ...
        setTimeout(() => {
            ...
            const newRows = rows.map((row) => {
                ...
                rules.forEach((rule) => {
                    ...
                    const targetKey = rule.newColumnSuffix ? `${rule.column}_${symOf(rule.to).replace("°", "")}` : rule.column;
            ...
            const filename = `${prefix.trim() || "temp_converted"}_${Date.now()}.xlsx`;
    ...
    return (
        <section className="flex flex-col gap-6">
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 px-4 py-3">
                <p className="text-xs text-zinc-500 uppercase tracking-wider mb-3 font-semibold">Live preview (value = 100)</p>
                <div className="flex flex-col gap-2">
                    {rules.map((rule) => {
                        const result = safeConvert(100, rule.from, rule.to, roundMode, decimals);
                        ...
                        return (
                            <div key={rule.id} className="flex items-center gap-2 text-sm flex-wrap">
                                <span className="font-mono text-zinc-300">100{symOf(rule.from)}</span>
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-2">
                    <input type="text" value={prefix} placeholder="temp_converted"
...
const INITIAL: AppState = {
    ...
    rules: [],
    outputOptions: { roundMode: "decimal", decimals: 2, filenamePrefix: "temp_converted" },
    stats: null,
};
        ...
        <div className="min-h-screen bg-zinc-950 text-zinc-100" style={{ fontFamily: "'IBM Plex Mono', ui-monospace, monospace" }}>
                    ...
                    <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded bg-amber-400 flex items-center justify-center">
                            <svg className="w-4 h-4 text-zinc-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M9 19V6.5a3 3 0 116 0V19a4 4 0 11-6 0z" />
                            </svg>
                        </div>
                        <span className="text-sm font-bold tracking-tight text-zinc-100">TempConvert</span>
            ...
            <footer className="border-t border-zinc-900 mt-16">
                <div className="max-w-2xl mx-auto px-6 py-4 flex items-center justify-between text-xs text-zinc-700">
                    <span>TempConvert · 5 units · CSV &amp; XLSX</span>

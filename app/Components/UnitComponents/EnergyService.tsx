...
/**
 * EnergyConverter.tsx — Self-contained, single-file energy unit converter
 * Same design system as PressureConverter (IBM Plex Mono, zinc/amber palette).
 *
 * Dependencies: papaparse, xlsx  (same as PressureConverter)
...
type EnergyUnit = "j" | "kj" | "mj" | "kwh" | "mwh" | "cal" | "kcal" | "btu" | "therm" | "ev";
type RoundMode = "exact" | "integer" | "decimal";
type Step = "upload" | "header" | "columns" | "output" | "done";

const ENERGY_UNITS: { value: EnergyUnit; label: string; symbol: string; category: string }[] = [
    { value: "j", label: "Joule", symbol: "J", category: "SI" },
    { value: "kj", label: "Kilojoule", symbol: "kJ", category: "SI" },
    { value: "mj", label: "Megajoule", symbol: "MJ", category: "SI" },
    { value: "kwh", label: "Kilowatt-hour", symbol: "kWh", category: "Electrical" },
    { value: "mwh", label: "Megawatt-hour", symbol: "MWh", category: "Electrical" },
    { value: "cal", label: "Calorie", symbol: "cal", category: "Thermal" },
    { value: "kcal", label: "Kilocalorie", symbol: "kcal", category: "Thermal" },
    { value: "btu", label: "BTU", symbol: "BTU", category: "Imperial" },
    { value: "therm", label: "Therm", symbol: "thm", category: "Imperial" },
    { value: "ev", label: "Electron-volt", symbol: "eV", category: "Scientific" },
];

/** Conversion factors to Joules (SI base) */
const TO_JOULE: Record<EnergyUnit, number> = {
    j: 1,
    kj: 1_000,
    mj: 1_000_000,
    kwh: 3_600_000,
...
function convertEnergy(value: number, from: EnergyUnit, to: EnergyUnit): number {
    if (from === to) return value;
    return (value * TO_JOULE[from]) / TO_JOULE[to];
}
...
function safeConvert(
    ...
    return applyRounding(convertEnergy(val, from, to), mode, decimals);
}
...
interface ConversionRule {
    ...
    from: EnergyUnit;
    to: EnergyUnit;
    /** Optional: append new column instead of overwriting */
                    ...
                    return (
                        <li key={step.id} className="flex items-center flex-1 last:flex-none">
                            <div className="flex flex-col items-center gap-1.5">
                                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all duration-300 ${isCurrent ? "bg-amber-400 border-amber-400 text-zinc-900 shadow-lg shadow-amber-400/30"
                                    : isPast || completed.includes(step.id) ? "bg-zinc-800 border-zinc-600 text-zinc-300"
    ...
    return (
        <section className="flex flex-col items-center gap-6">
            ...
            <label
                ...
                className={`relative w-full max-w-lg cursor-pointer rounded-xl border-2 border-dashed px-8 py-16 flex flex-col items-center gap-4 transition-all duration-200 select-none ${isDragging ? "border-amber-400 bg-amber-400/5 scale-[1.01]"
            ...
            >
                <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-colors ${isDragging ? "bg-amber-400/20" : "bg-zinc-800"}`}>
                    {loading ? (
                        ...
                        <svg className={`w-8 h-8 transition-colors ${isDragging ? "text-amber-400" : "text-zinc-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <div className="text-center">
                    ...
                    {!loading && <p className="text-xs text-zinc-600 mt-1">or <span className="text-amber-400 underline underline-offset-2">browse</span> to select</p>}
                </div>
                <input id="file-input" type="file" accept=".csv,.xlsx" className="sr-only"
                    onChange={(e) => { const f = e.target.files?.[0]; if (f) handle(f); e.target.value = ""; }} disabled={loading} />
            </label>
            {error && (
    ...
    return (
        <section className="flex flex-col gap-6">
            <div>
                ...
                <p className="text-sm text-zinc-500"><span className="text-zinc-400 font-medium">{filename}</span> · {rawRows.length} rows detected</p>
            </div>
                    ...
                    <tbody>
                        {rawRows.slice(0, PREVIEW).map((row, rIdx) => (
                            <tr key={rIdx} onClick={() => setSelected(rIdx)}
                                className={`cursor-pointer transition-colors border-b ${rIdx === selected ? "bg-amber-400/10 text-amber-300 font-semibold border-amber-400/30"
                                ...
                                <td className="px-3 py-1.5 text-zinc-600 font-mono">
                                    <span className={`inline-flex items-center gap-1 ${rIdx === selected ? "text-amber-400" : ""}`}>
                                        {rIdx === selected && <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>}
                                        {rIdx + 1}
                                    </span>
                                </td>
                        ...
                        {rawRows.length > PREVIEW && (
                            <tr><td colSpan={maxCols + 1} className="px-3 py-2 text-center text-zinc-700 text-xs italic">… {rawRows.length - PREVIEW} more rows</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            <div className="flex gap-3 pt-2">
                ...
                <button onClick={() => onConfirm(selected)} className="px-6 py-2.5 rounded-lg text-sm font-bold bg-amber-400 text-zinc-900 hover:bg-amber-300 transition-colors shadow-lg shadow-amber-400/20">
...
// ── Unit Select ────────────────────────────────────────────────────────────────
function UnitSelect({ value, onChange, label }: { value: EnergyUnit; onChange: (v: EnergyUnit) => void; label: string }) {
    const categories = [...new Set(ENERGY_UNITS.map((u) => u.category))];
    return (
        <div className="flex flex-col gap-1">
            <label className="text-xs text-zinc-500 font-medium">{label}</label>
            <select value={value} onChange={(e) => onChange(e.target.value as EnergyUnit)}
                className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-colors">
                {categories.map((cat) => (
                    <optgroup key={cat} label={cat}>
                        {ENERGY_UNITS.filter((u) => u.category === cat).map((u) => (
...
function ColumnsStep({ columns, sampleRows, initialRules, onConfirm, onBack }: {
    ...
    const addRule = () => {
        ...
        setRules((p) => [...p, { id: uid(), column: unused ?? columns[0] ?? "", from: "j", to: "kwh", newColumnSuffix: false }]);
        setError(null);
    };
    ...
    const validate = () => {
        ...
        const cols = rules.filter((r) => !r.newColumnSuffix).map((r) => r.column);
        if (cols.length !== new Set(cols).size) { setError("Each column can only appear once (or enable 'New column' to duplicate)."); return false; }
        return true;
    };

    const usedColumns = new Set(rules.filter((r) => !r.newColumnSuffix).map((r) => r.column));

    // Smart column detection — highlight columns that look like energy values
    ...
    return (
        <section className="flex flex-col gap-6">
            <div>
                ...
                <p className="text-sm text-zinc-500">Select which columns hold energy values and specify their current and target units.</p>
            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {suggestions.map((col) => (
                                <button key={col} onClick={() => {
                                    setRules((p) => [...p, { id: uid(), column: col, from: "j", to: "kwh", newColumnSuffix: false }]);
                                }}
                    ...
                    <div key={rule.id} className="group rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 hover:border-zinc-700 transition-colors">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <label className="flex items-center gap-1.5 cursor-pointer text-xs text-zinc-500 hover:text-zinc-300 transition-colors">
                                    <div onClick={() => update(rule.id, "newColumnSuffix", !rule.newColumnSuffix)}
                                        className={`w-8 h-4 rounded-full transition-colors relative ${rule.newColumnSuffix ? "bg-amber-400" : "bg-zinc-700"}`}>
                        ...
                        {rule.column && sampleRows[0] && sampleRows[0][rule.column] != null && (
                            <div className="mt-3 pt-3 border-t border-zinc-800 flex items-center gap-2 text-xs text-zinc-500 flex-wrap">
                                ...
                                {sampleRows.slice(0, 3).map((r, i) => {
                                    ...
                                    const sym = (u: string) => ENERGY_UNITS.find((x) => x.value === u)?.symbol ?? u;
                                    return (
                                        <span key={i} className="font-mono">
                                            <span className="text-zinc-400">{String(raw)} {sym(rule.from)}</span>
                                            <span className="text-zinc-700"> → </span>
                                            <span className="text-amber-400">{String(result)} {sym(rule.to)}</span>
                        ...
                        {rule.newColumnSuffix && (
                            <p className="mt-2 text-xs text-zinc-500">
                                A new column <span className="text-zinc-300 font-mono">{rule.column}_{ENERGY_UNITS.find(u => u.value === rule.to)?.symbol}</span> will be appended.
                            </p>
                        )}
                    </div>
                ))}
            ...
            <button onClick={addRule} disabled={rules.length >= columns.length}
                className="flex items-center gap-2 self-start px-4 py-2 rounded-lg text-sm font-medium border border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
...
function OutputStep({ rows, rules, initialOptions, onComplete, onBack }: {
    ...
    // Live unit preview
    const symOf = (v: string) => ENERGY_UNITS.find((u) => u.value === v)?.symbol ?? v;
        ...
        setTimeout(() => {
            ...
            const newRows = rows.map((row) => {
                ...
                rules.forEach((rule) => {
                    ...
                    const targetKey = rule.newColumnSuffix
                        ? `${rule.column}_${symOf(rule.to)}`
                        : rule.column;
            ...
            const filename = `${prefix.trim() || "energy_converted"}_${Date.now()}.xlsx`;
    ...
    return (
        <section className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-2">
                    <input type="text" value={prefix} placeholder="energy_converted"
                        onChange={(e) => setPrefix(e.target.value.replace(/[^a-zA-Z0-9_-]/g, ""))}
                        className="flex-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 placeholder-zinc-600 transition-colors" />
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 px-4 py-3">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                    {[
                        ...
                        { n: rules.length, label: "Column rules" },
...
const INITIAL: AppState = {
    ...
    rules: [],
    outputOptions: { roundMode: "decimal", decimals: 4, filenamePrefix: "energy_converted" },
    stats: null,
};
...
export default function EnergyConverter() {
    ...
    const onHeaderConfirm = useCallback((headerRowIndex: number) => {
        const headerRaw = state.rawRows[headerRowIndex] ?? [];
        const columns = deduplicateHeaders(headerRaw);
        ...
        patch({
            headerRowIndex, columns,
            rows: allRows,
            sampleRows: allRows.slice(0, 5),
            step: "columns",
            completed: mark(state.completed, "header"),
        });
        ...
        <div className="min-h-screen bg-zinc-950 text-zinc-100" style={{ fontFamily: "'IBM Plex Mono', ui-monospace, monospace" }}>
            ...
            <header className="border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-sm sticky top-0 z-10">
                <div className="max-w-2xl mx-auto px-6 h-14 flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-sm font-bold tracking-tight text-zinc-100">EnergyConvert</span>
            ...
            <footer className="border-t border-zinc-900 mt-16">
                <div className="max-w-2xl mx-auto px-6 py-4 flex items-center justify-between text-xs text-zinc-700">
                    <span>EnergyConvert · 10 units · CSV &amp; XLSX</span>

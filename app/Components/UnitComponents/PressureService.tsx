...
/**
 * PressureConverter.tsx — Self-contained, single-file pressure unit converter
...
const PRESSURE_UNITS: { value: PressureUnit; label: string; symbol: string; category: string }[] = [
    // SI
    { value: "pa", label: "Pascal", symbol: "Pa", category: "SI" },
    { value: "kpa", label: "Kilopascal", symbol: "kPa", category: "SI" },
    { value: "mpa", label: "Megapascal", symbol: "MPa", category: "SI" },
    { value: "gpa", label: "Gigapascal", symbol: "GPa", category: "SI" },
    // Bar
    { value: "bar", label: "Bar", symbol: "bar", category: "Metric" },
    { value: "mbar", label: "Millibar", symbol: "mbar", category: "Metric" },
    // Imperial
    { value: "psi", label: "Pound per sq in", symbol: "psi", category: "Imperial" },
    { value: "psf", label: "Pound per sq ft", symbol: "psf", category: "Imperial" },
    { value: "ksi", label: "Kilopound per sq in", symbol: "ksi", category: "Imperial" },
    // Atmospheric
    { value: "atm", label: "Standard atm", symbol: "atm", category: "Atmospheric" },
    { value: "at", label: "Technical atm", symbol: "at", category: "Atmospheric" },
    // Mercury / Water
    { value: "torr", label: "Torr", symbol: "Torr", category: "Mercury/Water" },
    { value: "mmhg", label: "Millimeter Hg", symbol: "mmHg", category: "Mercury/Water" },
    { value: "inhg", label: "Inch of Hg", symbol: "inHg", category: "Mercury/Water" },
    { value: "inwc", label: "Inch of water", symbol: "inWC", category: "Mercury/Water" },
    { value: "cmhg", label: "Centimeter Hg", symbol: "cmHg", category: "Mercury/Water" },
    { value: "cmwc", label: "Centimeter water", symbol: "cmWC", category: "Mercury/Water" },
];

/** Conversion factors to Pascal (SI base) */
const TO_PASCAL: Record<PressureUnit, number> = {
    pa: 1,
    kpa: 1_000,
    mpa: 1_000_000,
    gpa: 1_000_000_000,
...
function convertPressure(value: number, from: PressureUnit, to: PressureUnit): number {
    ...
    return (value * TO_PASCAL[from]) / TO_PASCAL[to];
}
...
function safeConvert(
    ...
    return applyRounding(convertPressure(val, from, to), mode, decimals);
}
...
const symOf = (v: string) => PRESSURE_UNITS.find((u) => u.value === v)?.symbol ?? v;
          ...
          ${loading ? "pointer-events-none opacity-60" : ""}`}
            ...
            <div className="w-full max-w-lg rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
                ...
                <ol className="flex flex-col gap-2">
                    {[
                        "Upload a CSV or Excel file with pressure data",
...
function UnitSelect({ value, onChange, label }: { value: PressureUnit; onChange: (v: PressureUnit) => void; label: string }) {
    const categories = [...new Set(PRESSURE_UNITS.map((u) => u.category))];
    return (
        <div className="flex flex-col gap-1">
            <label className="text-xs text-zinc-500 font-medium">{label}</label>
            <select value={value} onChange={(e) => onChange(e.target.value as PressureUnit)}
          ...
          focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-colors">
                {categories.map((cat) => (
                    <optgroup key={cat} label={cat}>
                        {PRESSURE_UNITS.filter((u) => u.category === cat).map((u) => (
...
function ColumnsStep({ columns, sampleRows, initialRules, onConfirm, onBack }: {
    ...
    const addRule = () => {
        ...
        setRules((p) => [...p, { id: uid(), column: unused ?? columns[0] ?? "", from: "bar", to: "psi", newColumnSuffix: false }]);
        setError(null);
    };
    ...
    return (
        <section className="flex flex-col gap-6">
            <div>
                ...
                <p className="text-sm text-zinc-500">Select which columns hold pressure values and specify their current and target units.</p>
            </div>
            ...
            {suggestions.length > 0 && (
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 px-4 py-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {suggestions.map((col) => (
                            <button key={col}
                                onClick={() => setRules((p) => [...p, { id: uid(), column: col, from: "bar", to: "psi", newColumnSuffix: false }])}
...
function OutputStep({ rows, rules, initialOptions, onComplete, onBack }: {
    ...
    const handleConvert = () => {
        ...
        setTimeout(() => {
            ...
            const filename = `${prefix.trim() || "pressure_converted"}_${Date.now()}.xlsx`;
        ...
        <section className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-2">
                    <input type="text" value={prefix} placeholder="pressure_converted"
...
const INITIAL: AppState = {
    ...
    rules: [],
    outputOptions: { roundMode: "decimal", decimals: 4, filenamePrefix: "pressure_converted" },
    stats: null,
};
        ...
        <div className="min-h-screen bg-zinc-950 text-zinc-100" style={{ fontFamily: "'IBM Plex Mono', ui-monospace, monospace" }}>
                    ...
                    <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded bg-amber-400 flex items-center justify-center">
                            ...
                            <svg className="w-4 h-4 text-zinc-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v.01M8.93 8.93l-.007.007M6 12H6.01M8.93 15.07l-.007.007M12 3a9 9 0 110 18A9 9 0 0112 3zm0 9l2.5-2.5" />
                            </svg>
                        </div>
                        <span className="text-sm font-bold tracking-tight text-zinc-100">PressureConvert</span>
            ...
            <footer className="border-t border-zinc-900 mt-16">
                <div className="max-w-2xl mx-auto px-6 py-4 flex items-center justify-between text-xs text-zinc-700">
                    <span>PressureConvert · 17 units · CSV &amp; XLSX</span>

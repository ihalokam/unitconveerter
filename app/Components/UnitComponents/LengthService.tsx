...
/**
 * LengthConverter.tsx — Self-contained, single-file length unit converter
 * Same design system as PressureConverter & EnergyConverter (IBM Plex Mono, zinc/amber palette).
 *
...
type LengthUnit =
    | "m" | "km" | "cm" | "mm" | "um" | "nm"
    | "in" | "ft" | "yd" | "mi" | "nmi"
    | "fathom" | "chain" | "furlong" | "lightyear" | "au";

type RoundMode = "exact" | "integer" | "decimal";
type Step = "upload" | "header" | "columns" | "output" | "done";

const LENGTH_UNITS: { value: LengthUnit; label: string; symbol: string; category: string }[] = [
    // SI / Metric
    { value: "nm", label: "Nanometer", symbol: "nm", category: "Metric" },
    { value: "um", label: "Micrometer", symbol: "μm", category: "Metric" },
    { value: "mm", label: "Millimeter", symbol: "mm", category: "Metric" },
    { value: "cm", label: "Centimeter", symbol: "cm", category: "Metric" },
    { value: "m", label: "Meter", symbol: "m", category: "Metric" },
    { value: "km", label: "Kilometer", symbol: "km", category: "Metric" },
    // Imperial / US
    { value: "in", label: "Inch", symbol: "in", category: "Imperial" },
    { value: "ft", label: "Foot", symbol: "ft", category: "Imperial" },
    { value: "yd", label: "Yard", symbol: "yd", category: "Imperial" },
    { value: "mi", label: "Mile", symbol: "mi", category: "Imperial" },
    { value: "chain", label: "Chain", symbol: "ch", category: "Imperial" },
    { value: "furlong", label: "Furlong", symbol: "fur", category: "Imperial" },
    // Nautical / Survey
    { value: "nmi", label: "Nautical Mile", symbol: "nmi", category: "Nautical" },
    { value: "fathom", label: "Fathom", symbol: "ftm", category: "Nautical" },
    // Scientific
    { value: "au", label: "Astronomical Unit", symbol: "AU", category: "Scientific" },
    { value: "lightyear", label: "Light-year", symbol: "ly", category: "Scientific" },
];

/** Conversion factors to Meters (SI base) */
...
function safeConvert(
    ...
    return applyRounding(convertLength(val, from, to), mode, decimals);
}
...
const symOf = (v: string) => LENGTH_UNITS.find((u) => u.value === v)?.symbol ?? v;
          ...
          ${loading ? "pointer-events-none opacity-60" : ""}`}
            ...
            <div className="w-full max-w-lg rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
                ...
                <ol className="flex flex-col gap-2">
                    {[
                        "Upload a CSV or Excel file with length/distance data",
...
function UnitSelect({ value, onChange, label }: { value: LengthUnit; onChange: (v: LengthUnit) => void; label: string }) {
    const categories = [...new Set(LENGTH_UNITS.map((u) => u.category))];
    return (
        <div className="flex flex-col gap-1">
            <label className="text-xs text-zinc-500 font-medium">{label}</label>
            <select value={value} onChange={(e) => onChange(e.target.value as LengthUnit)}
          ...
          focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-colors">
                {categories.map((cat) => (
                    <optgroup key={cat} label={cat}>
                        {LENGTH_UNITS.filter((u) => u.category === cat).map((u) => (
...
function ColumnsStep({ columns, sampleRows, initialRules, onConfirm, onBack }: {
    ...
    const addRule = () => {
        ...
        setRules((p) => [...p, { id: uid(), column: unused ?? columns[0] ?? "", from: "m", to: "ft", newColumnSuffix: false }]);
        setError(null);
    };
    ...
    return (
        <section className="flex flex-col gap-6">
            <div>
                ...
                <p className="text-sm text-zinc-500">Select which columns hold length/distance values and specify their current and target units.</p>
            </div>
            ...
            {suggestions.length > 0 && (
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 px-4 py-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {suggestions.map((col) => (
                            <button key={col}
                                onClick={() => setRules((p) => [...p, { id: uid(), column: col, from: "m", to: "ft", newColumnSuffix: false }])}
...
function OutputStep({ rows, rules, initialOptions, onComplete, onBack }: {
    ...
    const handleConvert = () => {
        ...
        setTimeout(() => {
            ...
            const filename = `${prefix.trim() || "length_converted"}_${Date.now()}.xlsx`;
        ...
        <section className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-2">
                    <input type="text" value={prefix} placeholder="length_converted"
...
const INITIAL: AppState = {
    ...
    rules: [],
    outputOptions: { roundMode: "decimal", decimals: 4, filenamePrefix: "length_converted" },
    stats: null,
};
        ...
        <div className="min-h-screen bg-zinc-950 text-zinc-100" style={{ fontFamily: "'IBM Plex Mono', ui-monospace, monospace" }}>
                    ...
                    <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded bg-amber-400 flex items-center justify-center">
                            ...
                            <svg className="w-4 h-4 text-zinc-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M3 7.5a1.5 1.5 0 011.5-1.5h15A1.5 1.5 0 0121 7.5v9a1.5 1.5 0 01-1.5 1.5h-15A1.5 1.5 0 013 16.5v-9zM7 9v3M10 9v1.5M13 9v3M16 9v1.5" />
                            </svg>
                        </div>
                        <span className="text-sm font-bold tracking-tight text-zinc-100">LengthConvert</span>
            ...
            <footer className="border-t border-zinc-900 mt-16">
                <div className="max-w-2xl mx-auto px-6 py-4 flex items-center justify-between text-xs text-zinc-700">
                    <span>LengthConvert · 16 units · CSV &amp; XLSX</span>
